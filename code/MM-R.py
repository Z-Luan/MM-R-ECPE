...
class Train_Dataset(object):
    def __init__(self, dataset_step1, dataset_step2, dataset_step3, dataset_emotion_clause_idx_list, dataset_emotion_cause_dic, dataset_cause_clause_idx_list, dataset_cause_emotion_dic):
    ...
    def __getitem__(self, index):
        return self.dataset_step1[index], self.dataset_step2[index], self.dataset_step3[index], self.dataset_emotion_clause_idx_list[index], self.dataset_emotion_cause_dic[index], self.dataset_cause_clause_idx_list[index], self.dataset_cause_emotion_dic[index]

    def __len__(self):
        return len(self.dataset_step1)
    
class Test_Dataset(object):
    def __init__(self, dataset_step1, dataset_emotion_clause_idx_list, dataset_emotion_cause_dic, dataset_cause_clause_idx_list, dataset_cause_emotion_dic, dataset_clauses, dataset_pairs):
        self.dataset_step1 = dataset_step1
        self.dataset_emotion_clause_idx_list = dataset_emotion_clause_idx_list
        self.dataset_emotion_cause_dic = dataset_emotion_cause_dic
        self.dataset_cause_clause_idx_list = dataset_cause_clause_idx_list
    ...
    def __getitem__(self, index):
        return self.dataset_step1[index], self.dataset_emotion_clause_idx_list[index], self.dataset_emotion_cause_dic[index], self.dataset_cause_clause_idx_list[index], self.dataset_cause_emotion_dic[index], self.dataset_clauses[index], self.dataset_pairs[index]

    def __len__(self):
        return len(self.dataset_step1)
    
dataset_folder_path = '/root/COLING-ECPE/code/data/split10/'
...
def load_train_dataset(dataset_type):
    ...
    for doc in dataset_total:
        ...
        static_emotion_query = "这是情感子句吗?" # step1
        step1_query_list.append(static_emotion_query)

        doc_len = int(doc['doc_len']) # 文本子句个数, e.g. 4
        doc_clauses = doc['clauses'] # 文本子句, e.g. [{'clause_id': '1', 'emotion_category': 'null', 'emotion_token': 'null', 'clause': '6月6日'}, {}]
        clause_list = []
        ...
        for pair in doc_pairs: # 一个情感子句可能对应多个原因子句 一个原因子句对应一个情感子句
            ...
            if (emotion_clause_idx - 1) not in emotion_cause_dic.keys():
                ...
                dynamic_cause_query = clause_list[emotion_clause_idx - 1] + '这句话对应的原因子句有哪些?' # step2
            ...
            dynamic_emotion_query = clause_list[cause_clause_idx - 1] + '这句话对应的情感子句是哪一句?' # step3
        ...
        step1_input_list, step1_seg_list, step1_query_span_list, step1_clause_span_list = combine(step1_query_list, clause_list)
        step2_input_list, step2_seg_list, step2_query_span_list, step2_clause_span_list = combine(step2_query_list, clause_list)
        step3_input_list, step3_seg_list, step3_query_span_list, step3_clause_span_list = combine(step3_query_list, clause_list)
    ...
    return Train_Dataset(dataset_step1, dataset_step2, dataset_step3, dataset_emotion_clause_idx_list, dataset_emotion_cause_dic, dataset_cause_clause_idx_list, dataset_cause_emotion_dic)
...
def load_test_dataset(dataset_type):
    ...
    for doc in dataset_total:
        ...
        static_emotion_query = "这是情感子句吗?" # step1 input
        step1_query_list.append(static_emotion_query)
    ...
    return Test_Dataset(dataset_step1, dataset_emotion_clause_idx_list, dataset_emotion_cause_dic, dataset_cause_clause_idx_list, dataset_cause_emotion_dic, dataset_clauses, dataset_pairs)
...
BERT_PATH = '/root/COLING-ECPE/code/pretrained_model/bert-base-chinese'
class Net(nn.Module):
    def __init__(self):
    ...
    def forward(self, step_type, idx, input_ids, attention_mask, token_type_ids, query_span, clause_span, emotion_clause_idx_list, emotion_cause_dic, cause_clause_idx_list, cause_emotion_dic):
        # input_ids (1, 300)
        ...
        if step_type == 'step1':
            bert_output = self.bert(input_ids = input_ids, token_type_ids = token_type_ids, attention_mask = attention_mask)[0]
    ...
    with torch.no_grad():
        ...
        with tqdm(total = test_dataloader_len) as test_bar:
            ...
            for step1, emotion_clause_idx_list, emotion_cause_dic, cause_clause_idx_list, cause_emotion_dic, clauses, pairs in test_dataloader:
                ...
                for idx, (step1_input, step1_seg, step1_query_span, step1_clause_span) in enumerate(zip(step1_input_list, step1_seg_list, step1_query_span_list, step1_clause_span_list)):
                    # input: ['[CLS]', '7', '月', '[SEP]', ...]
                    # seg: [0, ..., 0, 1, ..., 1]
                    # query_span: (1, 8)
                    # clause_span: [(10, 4), (14, 11), ...]
                    ...
                    # emotion_cause_dic: {9:[8]}
                    # cause_emotion_dic: {8:9}
                    ppl_result = tokenizer(step1_input, add_special_tokens = False, padding = 'max_length', truncation = True, max_length = truncation_length, is_split_into_words = True)
                    ...
                    pre, label = model(step_type, idx, input_ids.to(device), attention_mask.to(device), token_type_ids.to(device), step1_query_span, step1_clause_span, emotion_clause_idx_list, emotion_cause_dic, cause_clause_idx_list, cause_emotion_dic)
                ...
                if len(emotion_clause_pre_ture) == 0: # 未预测出真的情感子句
                    ...
                    for i in emotion_clause_pre_ture:
                        dynamic_cause_query = clauses[i] + '这句话对应的原因子句有哪些?' # step2
                        step2_query_list.append(dynamic_cause_query)

                    step2_input_list, step2_seg_list, step2_query_span_list, step2_clause_span_list = combine(step2_query_list, clauses)
 
                    for idx, (step2_input, step2_seg, step2_query_span, step2_clause_span) in enumerate(zip(step2_input_list, step2_seg_list, step2_query_span_list, step2_clause_span_list)):
                        ppl_result = tokenizer(step2_input, add_special_tokens = False, padding = 'max_length', truncation = True, max_length = truncation_length, is_split_into_words = True)
                        ...
                        pre, label = model(step_type, idx, input_ids.to(device), attention_mask.to(device), token_type_ids.to(device), step2_query_span, step2_clause_span, emotion_clause_pre_ture, emotion_cause_dic, cause_clause_idx_list, cause_emotion_dic)
                ...
                if len(emotion_clause_pre_ture) == 0: # 未预测出真的情感子句
                    ...
                    for i in cause_clause_pre:
                        dynamic_emotion_query = clauses[i] + '这句话对应的情感子句是哪一句?' # step3
                        step3_query_list.append(dynamic_emotion_query)

                    step3_input_list, step3_seg_list, step3_query_span_list, step3_clause_span_list = combine(step3_query_list, clauses)

                    for idx, (step3_input, step3_seg, step3_query_span, step3_clause_span) in enumerate(zip(step3_input_list, step3_seg_list, step3_query_span_list, step3_clause_span_list)):
                        ppl_result = tokenizer(step3_input, add_special_tokens = False, padding = 'max_length', truncation = True, max_length = truncation_length, is_split_into_words = True)
                        input_ids = torch.tensor(ppl_result['input_ids']).unsqueeze(0)
                        attention_mask = torch.tensor(ppl_result['attention_mask']).unsqueeze(0)
                        step3_seg = step3_seg + [1] * (truncation_length - len(step3_seg))
                        token_type_ids = torch.tensor(step3_seg).unsqueeze(0)

                        pre, _ = model(step_type, idx, input_ids.to(device), attention_mask.to(device), token_type_ids.to(device), step3_query_span, step3_clause_span, emotion_clause_idx_list, emotion_cause_dic, cause_clause_idx_list, cause_emotion_dic)
        ...
        print('Test Emotion Precision:', average_emotion_precision, ' Test Emotion Recall:', average_emotion_recall, ' Test Emotion F1', average_emotion_f1)
        print('Test Cause Precision:', average_cause_precision, ' Test Cause Recall:', average_cause_recall, ' Test Cause F1', average_cause_f1)
        print('Test Pair Precision:', average_pair_precision, ' Test Pair Recall:', average_pair_recall, ' Test Pair F1', average_pair_f1)
...
def train(model, train_dataset, test_dataset):
    ...
    #                 pre, label = model(step_type, idx, input_ids.to(device), attention_mask.to(device), token_type_ids.to(device), step2_query_span, step2_clause_span, emotion_clause_idx_list, emotion_cause_dic, cause_clause_idx_list, cause_emotion_dic)
    #                 pre_list.append(pre)
    ...
    #                 pre, label = model(step_type, idx, input_ids.to(device), attention_mask.to(device), token_type_ids.to(device), step3_query_span, step3_clause_span, emotion_clause_idx_list, emotion_cause_dic, cause_clause_idx_list, cause_emotion_dic)
    #                 pre_list.append(pre)
    #                 label_list.append(label)
